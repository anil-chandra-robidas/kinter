...
$banner_overlay           = "";
   ...
   if ( defined( 'DEVM' ) ) {
    $blog_show_banner          = kinter_option('blog_show_banner');
    if ( $blog_show_banner !== 'yes' ) {
        return;
    }

   $banner_settings         = kinter_option('blog_banner_setting');
   $blog_show_breadcrumb    = kinter_option('blog_show_breadcrumb');
   $blog_banner_title       = kinter_option('blog_banner_title');
   $banner_blog_image       = kinter_option('banner_blog_image');
   $banner_overlay          = kinter_option('show_blog_banner_overlay');
   $blog_banner_title_color = kinter_option('blog_banner_title_color');
...
<section class="<?php echo esc_attr($wraper_class); echo esc_attr($banner_image == '' ?' banner-solid':' banner-bg'); ?>" style="background-image: url(<?php echo esc_attr( $banner_image ); ?>)">
    <?php if ($banner_overlay === 'yes') {
		...
		if ( defined( 'DEVM' ) ) {
			$banner_overlay_color = kinter_option("blog_banner_overlay_color");
		}
        ?>
        ...
        <div class="row">
            <div class="col-12">
                <div class="xs-jumbotron-content-wraper">
                    ...
                    <?php if(isset($show_breadcrumb) && $show_breadcrumb == 'yes' && !is_home(  )): ?>
